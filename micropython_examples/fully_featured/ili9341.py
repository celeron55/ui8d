...
class Display(object):
    ...
    def draw_text(self, x, y, text, font, color,  background=0,
                  ...
                  force_pixel_length=None, right_align=False):
        """Draw text.
        ...
        if force_pixel_length is not None and right_align:
            ...
            if total_length < force_pixel_length:
                length_to_add = force_pixel_length - total_length
                # Fill in the rest with background color. This allows masking the
                # old value, which could have been different length
                if landscape:
                    self.fill_hrect(x, y - length_to_add, h, length_to_add, background)
                    y -= length_to_add
                else:
                    self.fill_hrect(x, y, length_to_add, h, background)
                    x += length_to_add
        ...
        if force_pixel_length is not None and current_length < force_pixel_length:
            length_to_add = force_pixel_length - current_length
            # Fill in the rest with background color. This allows masking the
            # old value, which could have been different length
            if landscape:
                self.fill_hrect(x, y - length_to_add, h, length_to_add, background)
