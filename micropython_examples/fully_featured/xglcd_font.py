...
class XglcdFont(object):
    ...
    Attributes:
        ...
        special_letters: List of letters at the end of the list to be specially mapped
    ...
    def __init__(self, path, width, height, start_letter=32, letter_count=97, special_letters=['°']):
    ...
    def lit_bits(self, n):
        position = 0
        while n:
            b = n & -n  # Isolate the least significant bit set
            # Manually calculate bit length
            bit_length = 0
            temp = b
            while temp:
                temp >>= 1
                bit_length += 1
            position = (bit_length - 1) * 2  # Convert bit position to byte offset
            yield position
            n ^= b  # Clear the bit we've just processed

    def get_index_of_letter(self, letter):
        letter_ord = ord(letter) - self.start_letter
        if letter_ord >= self.letter_count - len(self.special_letters):
            if letter in self.special_letters:
                return self.letter_count - len(self.special_letters) + self.special_letters.index(letter)
            else:
                return None
        return letter_ord
    ...
    def get_letter(self, letter, color, background=0, landscape=False):
        ...
        """
        # Get index of letter
        letter_ord = self.get_index_of_letter(letter)
        # Confirm font contains letter
        if letter_ord is None or letter_ord >= self.letter_count:
