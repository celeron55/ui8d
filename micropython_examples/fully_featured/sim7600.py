...
class Sim:
    ...
    async def http_get(self, url, timeout_ms=30000, response_max_len=1000):
        ...
        read_len = min(content_length, response_max_len)

        response = await self.send_command("AT+HTTPREAD=0,"+str(read_len), response_max_len=read_len+100) # Read bytes
        header = b'AT+HTTPREAD=0,'+str(read_len)+'\r\r\nOK\r\n\r\n+HTTPREAD: DATA,'+str(read_len)+'\r\n'
