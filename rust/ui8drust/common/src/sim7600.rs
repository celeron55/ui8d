...
static REQUEST_STEPS: [RequestStep; 11] = [
    RequestStep {
        command: "AT+CPIN?\r",
        timeout_ms: 1000,
        max_retry_count: 60,
    ...
    RequestStep {
        ...
        on_parse: |_step: &RequestStep,
         ...
         -> HttpUpdateStatus {
            ...
            if !driver.rxbuf.contains(&*required_header) {
                // Waiting for header
            ...
            let data_end = driver.rxbuf.find(end_marker);
            if data_end.is_none() {
                // Waiting for header
                info!("SIM7600: AT+HTTPREAD: Waiting for end marker");
                return HttpUpdateStatus::Processing;
            }

            let data_end = data_end.unwrap();
            info!("SIM7600: data_end: {:?}", data_end);

            let body = &driver.rxbuf[required_header.len()..data_end];
    ...
    pub fn http_get_update(&mut self) -> HttpUpdateStatus {
        if let Some(request) = &mut self.request {
            if let Some(step_i) = request.step_i {
                ...
                self.buffers.rxbuf.clear();
                HttpUpdateStatus::Failed(HttpFailReason::InternalError)
            }
        } else {
